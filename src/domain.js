...
export function metrics(rows) {
  ...
  const categories = Object.create(null); const months = Object.create(null);
...
export function previousPeriod(from,to) { if(!validDate(from)||!validDate(to)||from>to)return {from:'',to:''}; const days = Math.round((Date.parse(to)-Date.parse(from))/86400000)+1; const end = new Date(Date.parse(from)-86400000); return {from:new Date(+end-(days-1)*86400000).toISOString().slice(0,10),to:end.toISOString().slice(0,10)}; }
